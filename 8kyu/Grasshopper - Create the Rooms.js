/* You are creating an escape the room game. The first step is create an object called rooms 
that contains all of the rooms of the game. The room object should have at least 3 rooms as properties,
and each individial room should have at least three properties (example: name, description, completed).


*/

//Solution following the exercise
var rooms = {
  firstRoom: {
    name: 'Zombie',
    description: 'Yep, there are zombies.',
    completed: false
  },
  
  secondRoom: {
    name: 'Post-apocalyptic',
    description: 'All dead.',
    completed: false  
  },
  
  thirdRoom: {
    name: 'Prison',
    description: 'You didn\'t have an alibi, sorry.',
    completed: true
  }
}
